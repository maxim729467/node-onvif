  ...
  let promise = new Promise((resolve, reject) => {
    ...
    this.getVideoEncoderConfiguration({ ConfigurationToken: params['ConfigurationToken'] })
      .then((result) => {
        ...
        // Encoding
        if (params.Encoding) {
          if (['JPEG', 'MPEG4', 'H264', 'H265'].indexOf(params.Encoding) === -1) {
            reject(new Error('Invalid Encoding. Must be one of: JPEG, MPEG4, H264, H265'));
            return;
          }
        ...
        // H264 Configuration
        if (params.Encoding === 'H264' || (currentConfig.Encoding === 'H264' && params.Encoding !== 'H265')) {
          soap_body += '<tt:H264>';
        ...
        // H265 Configuration
        else if (params.Encoding === 'H265' || currentConfig.Encoding === 'H265') {
          soap_body += '<tt:H265>';

          // Gov Length
          if (params.GovLength !== undefined) {
            soap_body += '<tt:GovLength>' + params.GovLength + '</tt:GovLength>';
          } else if (params.H265 && params.H265.GovLength !== undefined) {
            soap_body += '<tt:GovLength>' + params.H265.GovLength + '</tt:GovLength>';
          } else if (currentConfig.H265 && currentConfig.H265.GovLength) {
            soap_body += '<tt:GovLength>' + currentConfig.H265.GovLength + '</tt:GovLength>';
          } else {
            soap_body += '<tt:GovLength>30</tt:GovLength>'; // Default value
          }

          // H265 Profile
          if (params.H265 && params.H265.H265Profile) {
            if (['Main', 'Main10', 'MainStill'].indexOf(params.H265.H265Profile) === -1) {
              throw new Error('Invalid H265Profile. Must be one of: Main, Main10, MainStill');
            }
            soap_body += '<tt:H265Profile>' + params.H265.H265Profile + '</tt:H265Profile>';
          } else if (currentConfig.H265 && currentConfig.H265.H265Profile) {
            soap_body += '<tt:H265Profile>' + currentConfig.H265.H265Profile + '</tt:H265Profile>';
          } else {
            soap_body += '<tt:H265Profile>Main</tt:H265Profile>'; // Default value
          }
      ...
      .catch((error) => {
        // Check if it's a 400 error - try again with ForcePersistence
        if (error && error.message && error.message.includes('400 Bad Request')) {
          console.log('[ONVIF] Initial request failed with 400, retrying with ForcePersistence flag');

          // Get the current configuration again
          this.getVideoEncoderConfiguration({ ConfigurationToken: params['ConfigurationToken'] })
            .then((result) => {
              if (!result || !result.data || !result.data.GetVideoEncoderConfigurationResponse || !result.data.GetVideoEncoderConfigurationResponse.Configuration) {
                throw new Error('Failed to get current video encoder configuration');
              ...
              // Start building the SOAP body with ForcePersistence
              let soap_body = '';
              soap_body += '<trt:SetVideoEncoderConfiguration>';
              soap_body += '<trt:Configuration token="' + params['ConfigurationToken'] + '">';
              ...
              if (params.Encoding) {
                if (['JPEG', 'MPEG4', 'H264', 'H265'].indexOf(params.Encoding) === -1) {
                  throw new Error('Invalid Encoding. Must be one of: JPEG, MPEG4, H264, H265');
                }
                soap_body += '<tt:Encoding>' + params.Encoding + '</tt:Encoding>';
              } else {
                soap_body += '<tt:Encoding>' + (currentConfig.Encoding || 'H264') + '</tt:Encoding>';
              ...
              // Frame rate and Bitrate (RateControl)
              soap_body += '<tt:RateControl>';
              if (params.FrameRate !== undefined) {
                soap_body += '<tt:FrameRateLimit>' + params.FrameRate + '</tt:FrameRateLimit>';
              } else if (currentConfig.RateControl && currentConfig.RateControl.FrameRateLimit) {
                soap_body += '<tt:FrameRateLimit>' + currentConfig.RateControl.FrameRateLimit + '</tt:FrameRateLimit>';
              } else {
                soap_body += '<tt:FrameRateLimit>30</tt:FrameRateLimit>';
              }
              ...
              // H265 Configuration
                ...
                if (params.H265 && params.H265.H265Profile) {
                  if (['Main', 'Main10', 'MainStill'].indexOf(params.H265.H265Profile) === -1) {
                    reject(new Error('Invalid H265Profile. Must be one of: Main, Main10, MainStill'));
                    return;
                  }
                  soap_body += '<tt:H265Profile>' + params.H265.H265Profile + '</tt:H265Profile>';
                } else if (currentConfig.H265 && currentConfig.H265.H265Profile) {
              ...
              // Add ForcePersistence flag for the retry
              ...
              // Try again with ForcePersistence
              return mOnvifSoap.requestCommand(this.getRequestParams(), 'SetVideoEncoderConfiguration', soap);
            })
            .then((result) => {
              console.log('[ONVIF] Second attempt with ForcePersistence succeeded');
              resolve(result);
            })
            .catch((retryError) => {
              console.error('[ONVIF] Both attempts failed:', retryError.message);
              reject(retryError);
            });
        } else {
