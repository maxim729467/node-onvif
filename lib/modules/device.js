      ...
      const profilePromises = this.profile_list.map(async (profile, profileIndex) => {
        ...
        try {
          ...
          if (
            ...
            // Get imaging and video encoder options in parallel
            const optionsPromises = [];

            // Get imaging options if service is available
            if (this.services['imaging']) {
              optionsPromises.push(
                this.getImagingOptions(profileIndex)
                  .then((imagingResult) => {
                    if (imagingResult && imagingResult.data && imagingResult.data.GetOptionsResponse && imagingResult.data.GetOptionsResponse.ImagingOptions) {
                      capabilities.imaging = this._parseImagingOptions(imagingResult.data.GetOptionsResponse.ImagingOptions);
                    }
                    // Also get current imaging settings
                    return this.getImagingSettings(profileIndex);
                  })
                  .then((imagingSettings) => {
                    if (imagingSettings && imagingSettings.data && imagingSettings.data.GetImagingSettingsResponse) {
                      capabilities.currentImagingSettings = imagingSettings.data.GetImagingSettingsResponse.ImagingSettings;
                    }
                  })
                  .catch((error) => {
                    console.error(`\n[ONVIF] Error getting imaging options for profile ${profileIndex}: ${error.message}`);
                  })
              );
            }
            ...
            if (this.services['media']) {
              optionsPromises.push(
                this.getVideoEncoderConfigurationOptions(profileIndex)
                  .then((encoderOptionsResult) => {
            ...
            // Wait for all options to be processed
