...
OnvifDevice.prototype._getSystemDateAndTime = function () {
  let promise = new Promise((resolve, reject) => {
    this.services.device.getSystemDateAndTime((error, result) => {
      ...
      console.log('[ONVIF] Time diff with device:', this.time_diff, 'ms');
      resolve();
    });
  });
  return promise;
...
OnvifDevice.prototype._getCapabilities = function () {
  let promise = new Promise((resolve, reject) => {
    this.services.device.getCapabilities((error, result) => {
      ...
      console.log('[ONVIF] Capabilities:', JSON.stringify({
        Events: c['Events'] && c['Events']['XAddr'],
        Imaging: c['Imaging'] && c['Imaging']['XAddr'],
        Media: c['Media'] && c['Media']['XAddr'],
        PTZ: c['PTZ'] && c['PTZ']['XAddr'],
        Device: c['Device'] && c['Device']['XAddr'],
      }));
      let events = c['Events'];
  ...
  let promise = new Promise((resolve, reject) => {
    this.services.device.getDeviceInformation((error, result) => {
      if (error) {
        ...
        console.log('[ONVIF] Device info:', JSON.stringify(this.information));
        resolve();
      }
    });
  });
