...
function checkStreamCodec(url, options) {
  return new Promise((resolve, reject) => {
    const authUrl = processStreamUrl(url, options);
    const ffmpegProcess = spawn('ffprobe', [
      '-v', 'error',
      '-i', authUrl,
      '-rtsp_transport', 'tcp',
      '-timeout', 10000000,
    ...
    ffmpegProcess.on('close', (code) => {
      ...
      try {
        const metadata = JSON.parse(ffmpegOutput);
        // console.log(metadata);
        if (!metadata || !metadata.streams) {
          reject(new Error('No stream metadata'));
        }

        const videoStream = metadata.streams.find(stream => stream.codec_type === 'video' && /(h264|hevc)/.test(stream.codec_name));
        resolve(Boolean(videoStream));
      } catch (err) {
        reject(err);
      }
 ...
 * ---------------------------------------------------------------- */
OnvifDevice.prototype.getUdpStreamUrl = async function (options) {
  ...
  // ======= WITHOUT FFPROBE =======
  let info = _.uniqBy(streams, 'url').filter(stream => stream.url);
  console.log({ onvifInfo: info });
  return info;

  // ======= WITH FFPROBE =======
  // info = await Promise.all(info.map(async streamData => {
  //   try {
  //     const isVideoStream = await checkStreamCodec(streamData.url, options);
  //     return isVideoStream ? streamData : null;
  //   } catch (err) {
  //     console.log(err);
  //     return null;
  //   }
  // }));
  //
  // const filteredChannels = info.filter(item => Boolean(item));
