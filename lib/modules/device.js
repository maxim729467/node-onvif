...
OnvifDevice.prototype.getUdpStreamUrl = function () {
  ...
  const streams = this.profile_list.map((profile, index) => {
    ...
    streamData.isMainStream = index ? false : true;
    ...
    // console.log({urlIpAddressArr})
    if (urlIpAddressArr) urlIpAddress = urlIpAddressArr[1];
    if (urlIpAddress) streamData.url = streamData.url.replace(urlIpAddress, ipAddressOrHost);
    // console.log({urlIpAddress});

    streamData.fps = encoder.framerate || null;
    // streamData.codec = encoder.encoding || "H265";
    streamData.resolution = encoder.resolution ? `${encoder.resolution.width}x${encoder.resolution.height}` : null;
    // if (this.information && this.information.Manufacturer && this.information.Manufacturer.toLowerCase().includes("holowits")) {
    //   streamData.codec = null;
    // }
    return streamData;
  });

  return { streams: streams.length ? _.uniqBy(streams, 'url') : [] };
};
