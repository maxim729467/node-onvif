...
* }
* ---------------------------------------------------------------- */
OnvifServicePtz.prototype.continuousMove = function(params, information, callback) {
	let promise = new Promise((resolve, reject) => {
		let err_msg = '';
		if(err_msg = mOnvifSoap.isInvalidValue(params, 'object')) {
			reject(new Error('The value of "params" was invalid: ' + err_msg));
		...
		soap_body += '</tptz:ContinuousMove>';
		let soap = this._createRequestSoap(soap_body);
		mOnvifSoap.requestCommand(this.getRequestParams(), 'ContinuousMove', soap, information).then((result) => {
			resolve(result);
		}).catch((error) => {
			reject(error);
		});
...
* }
* ---------------------------------------------------------------- */
OnvifServicePtz.prototype.stop = function(params, information, callback) {
	let promise = new Promise((resolve, reject) => {
		let err_msg = '';
		if(err_msg = mOnvifSoap.isInvalidValue(params, 'object')) {
			reject(new Error('The value of "params" was invalid: ' + err_msg));
		...
		mOnvifSoap.requestCommand(this.getRequestParams(), 'Stop', soap, information).then((result) => {
			resolve(result);
		}).catch((error) => {
			reject(error);
		});
...
OnvifServicePtz.prototype.getPresets = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		...
		mOnvifSoap.requestCommand(this.getRequestParams(), 'GetPresets', soap).then((result) => {
			try {
				let resp = result['data']['GetPresetsResponse'];
				if(resp && resp['Preset'] && !Array.isArray(resp['Preset'])) {
					resp['Preset'] = [resp['Preset']];
				}
			} catch(e) {}
			resolve(result);
		}).catch((error) => {
