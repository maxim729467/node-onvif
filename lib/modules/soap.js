...
OnvifSoap.prototype._request = async function(oxaddr, soap, method_name, information = {}) {
	...
	if ((['GetProfiles', 'GetStreamUri'].includes(method_name)) && information['Manufacturer'] === 'PAVANA') {
		oxaddr.path = oxaddr.path.replace('device', 'media');
		oxaddr.pathname = oxaddr.pathname.replace('device', 'media');
		oxaddr.href = oxaddr.href.replace('device', 'media');
	}

	if ((['ContinuousMove', 'Stop'].includes(method_name)) && information['Manufacturer'] === 'PAVANA') {
		oxaddr.path = oxaddr.path.replace('device', 'ptz');
		oxaddr.pathname = oxaddr.pathname.replace('device', 'ptz');
		oxaddr.href = oxaddr.href.replace('device', 'ptz');
	}

	// console.log({ method_name, information, pathname: oxaddr.pathname });
