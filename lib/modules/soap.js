...
OnvifSoap.prototype.requestCommand = function (oxaddr, method_name, soap, information) {
  let promise = new Promise((resolve, reject) => {
    let xml = '';
    this._request(oxaddr, soap, method_name, information)
      .then((res) => {
        xml = res;
...
OnvifSoap.prototype._request = async function (oxaddr, soap, method_name, information = {}) {
  ...
  const manufacturer = (information['Manufacturer'] || '').toUpperCase();
  if (['PAVANA', 'ZENITEL', 'VIVOTEK'].includes(manufacturer)) {
  ...
  let promise = new Promise((resolve, reject) => {
    ...
    if (manufacturer === 'VIVOTEK') {
      const mediaMethods = ['GetProfiles', 'GetStreamUri', 'GetSnapshotUri', 'GetVideoEncoderConfigurations', 'SetVideoEncoderConfiguration'];
      const ptzMethods = ['ContinuousMove', 'Stop', 'AbsoluteMove', 'RelativeMove', 'GetConfigurations'];
      let svcPath = 'device';
      if (mediaMethods.includes(method_name)) svcPath = 'media';
      else if (ptzMethods.includes(method_name)) svcPath = 'ptz';
      headers['SOAPAction'] = `http://www.onvif.org/ver10/${svcPath}/wsdl/${method_name}`;
    }
    ...
    let req = httpLib.request(opts, (res) => {
      ...
      res.on('end', () => {
        ...
        console.log('[ONVIF SOAP] <== ', method_name, res.statusCode, res.statusMessage, 'headers:', JSON.stringify(res.headers));
        if (res.statusCode === 200) {
          console.log('[ONVIF SOAP] Response body:', xml);
          resolve(xml);
        } else {
          console.log('[ONVIF SOAP] <== ', method_name, 'FAILED', res.statusCode, res.statusMessage);
          console.log('[ONVIF SOAP] Response body:', xml);
